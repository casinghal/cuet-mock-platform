...
function buildTestPrompt(mode, diff) {
  ...
  const ddesc = {
    easy:"simple vocabulary, direct factual questions, obvious inference",
    medium:"moderate vocabulary above HSC level, some inference required, plausible distractors",
    hard:"advanced GRE-level vocabulary, complex literary passages, abstract inference, tricky distractors"
  }[diff];
...
- Sentence Rearrangement: ${d.sr} questions — four sentences labeled P, Q, R, S, options are 4 distinct orderings (e.g. "PQRS", "QPRS", "SRPQ", "RPQS")
...
4. Passages go in a SEPARATE "passages" array — questions reference by passage_id only (DO NOT repeat passage text in questions)
5. Sentence rearrangement: show P, Q, R, S as separate labelled sentences in the question, options are orderings
6. Explanation: 1-2 sentences max, clear and educational

Return ONLY valid compact JSON, no markdown, no preamble:
{"passages":[{"id":"P1","type":"factual","text":"full passage text here"}],"questions":[{"id":1,"topic":"reading_comprehension","passage_id":"P1","question":"Based on the passage...?","options":{"A":"...","B":"...","C":"...","D":"..."},"correct":"A","explanation":"The passage states..."},{"id":${d.rc[1]+1},"topic":"synonyms_antonyms","question":"Choose the SYNONYM of ELOQUENT:","options":{"A":"Silent","B":"Articulate","C":"Confused","D":"Verbose"},"correct":"B","explanation":"Eloquent means fluent and persuasive; articulate is its closest synonym."}]}`;
}

function buildAdvisoryPrompt(res, name) {
  const tperf = Object.entries(res.topicStats)
    .sort(([,a],[,b])=>(a.correct/a.total||0)-(b.correct/b.total||0))
    .map(([t,s])=>`${TOPICS[t]?.label||t}: ${s.correct}/${s.total} correct (${s.total?Math.round(s.correct/s.total*100):0}%)`).join("\n");
  const wrongs = (res.questionResults||[]).filter(q=>q.status==="wrong").slice(0,6)
    .map(q=>`[${TOPICS[q.topic]?.abbr||q.topic}] "${(q.question||"").slice(0,65)}..." — chose ${q.userAnswer}, correct: ${q.correct}`).join("\n");
  return `CUET English coaching session. Student: ${name} (Class 12, CUET UG 2026, English subject 101).

Latest test score: ${res.score}/${res.maxScore} (${res.percentage}%)
...
Topic performance (sorted weakest to strongest):
${tperf}

Sample wrong answers (first 6):
${wrongs||"None — all wrong answers were skipped"}

Write a coaching advisory in 300-360 words. Four paragraphs, no bullets, no headers, no numbered lists:

Para 1: Honest one-paragraph assessment. State the score, what it means in CUET context, what it reveals about preparation. Not a pep talk.
Para 2: The 2-3 weakest areas specifically — what the errors reveal about the gap, and exactly what practice exercise will fix each gap.
Para 3: Attempt strategy for the next test — which topic type to start with, approximate time per question by topic, when to skip and come back, the math on skipping vs guessing wrong.
Para 4: One specific daily practice habit for the next 14 days. Concrete resource or activity, not a platitude.

Write like a results-focused exam coach who respects the student's intelligence. No "great effort", no "keep going". Only what will move the score.`;
}
...
// ─── Direct Anthropic API call (key stored in localStorage) ─
async function callAI(prompt, max_tokens) {
  const key = localStorage.getItem("cuet_api_key");
  if (!key) throw new Error("API key not configured. Please set up your Anthropic API key.");
...
export default function CUETPlatform() {
  const [apiKey,setApiKey]=useState(()=>localStorage.getItem("cuet_api_key")||"");
  ...
  const saveApiKey=()=>{
    if(!setupKey.trim()){setSetupErr("Please enter your Anthropic API key.");return;}
    if(!setupKey.trim().startsWith("sk-ant-")){setSetupErr("Invalid key format. Anthropic API keys start with sk-ant-");return;}
    localStorage.setItem("cuet_api_key",setupKey.trim());
    setApiKey(setupKey.trim());setSetupErr("");setShowKeySettings(false);
  };
  const handleLogout=()=>{clearInterval(timerRef.current);setUser(null);setScreen("auth");setAuthForm({name:"",email:"",password:""});setShowKeySettings(false);};
  ...
  const goTo=useCallback((idx)=>{
    ...
    setCurrentQ(idx);
    setSelectedOpt(answers[questions[idx]?.id]||null);
  },[questions,answers]);
  ...
  const clearResponse=()=>{
    const q=questions[currentQ];
    setSelectedOpt(null);
    if(q) setAnswers(a=>{const n={...a};delete n[q.id];return n;});
    if(q) setMarkedReview(m=>{const s=new Set(m);s.delete(q.id);return s;});
  };
  ...
  // ═══════════════════════════════════════════════════════════
  //  SETUP SCREEN — first-time API key configuration
  // ═══════════════════════════════════════════════════════════
  if(!apiKey) return(
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0f172a 0%,#1e3a8a 55%,#1d4ed8 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      <div style={{marginBottom:20,textAlign:"center"}}>
        ...
        <div style={{color:"rgba(255,255,255,0.5)",fontSize:12}}>English Mock Test Platform — First-Time Setup</div>
      </div>
      <div style={{background:"white",borderRadius:14,padding:"30px 34px",width:"100%",maxWidth:420,boxShadow:"0 24px 64px rgba(0,0,0,0.45)"}}>
        <div style={{textAlign:"center",marginBottom:22}}>
          <div style={{fontSize:22,fontWeight:900,color:"#0f172a"}}>One-Time Setup</div>
          <div style={{fontSize:13,color:"#64748b",marginTop:4,lineHeight:1.6}}>Enter your Anthropic API key to activate AI test generation. This is stored only on this device.</div>
        </div>
        <div style={{background:"#f0f7ff",border:"1px solid #bfdbfe",borderRadius:8,padding:"12px 14px",marginBottom:20,fontSize:12,color:"#1e40af",lineHeight:1.6}}>
        ...
        <div style={{marginBottom:setupErr?"12px":"20px"}}>
          <label style={{display:"block",fontSize:12,fontWeight:700,color:"#374151",marginBottom:6,textTransform:"uppercase",letterSpacing:0.5}}>Anthropic API Key</label>
          <input value={setupKey} onChange={e=>setSetupKey(e.target.value)} onKeyDown={e=>e.key==="Enter"&&saveApiKey()}
            placeholder="sk-ant-api03-..."
            style={{width:"100%",padding:"11px 12px",border:"1.5px solid #e2e8f0",borderRadius:7,fontSize:13,boxSizing:"border-box",outline:"none",fontFamily:"monospace"}}/>
        </div>
        {setupErr&&<div style={{background:"#fef2f2",border:"1px solid #fecaca",color:"#dc2626",padding:"9px 12px",borderRadius:6,fontSize:13,marginBottom:16,lineHeight:1.4}}>{setupErr}</div>}
        <button onClick={saveApiKey}
          style={{width:"100%",padding:"13px",background:"linear-gradient(135deg,#1e3a8a,#1d4ed8)",color:"white",border:"none",borderRadius:8,fontSize:15,fontWeight:800,cursor:"pointer"}}>
          Activate Platform →
        </button>
        <div style={{marginTop:14,fontSize:11,color:"#94a3b8",textAlign:"center",lineHeight:1.6}}>
          Your key is stored locally on this device only. It is never sent to any third-party server.
        </div>
      </div>
    </div>
  );
  ...
  if(screen==="auth") return(
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0f172a 0%,#1e3a8a 55%,#1d4ed8 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      <div style={{marginBottom:20,textAlign:"center"}}>
        ...
        <div style={{color:"rgba(255,255,255,0.5)",fontSize:12,letterSpacing:0.5}}>English — Section I (Language) · Mock Test Platform</div>
      </div>
      <div style={{background:"white",borderRadius:14,padding:"30px 34px",width:"100%",maxWidth:390,boxShadow:"0 24px 64px rgba(0,0,0,0.45)"}}>
        ...
        <div style={{display:"flex",background:"#f1f5f9",borderRadius:7,padding:3,marginBottom:22,gap:3}}>
          {[["login","Sign In"],["register","Register"]].map(([m,l])=>(
            <button key={m} onClick={()=>{setAuthMode(m);setAuthErr("");}}
              style={{flex:1,padding:"9px 0",border:"none",borderRadius:5,cursor:"pointer",fontWeight:700,fontSize:13,
                background:authMode===m?"white":"transparent",color:authMode===m?"#1e3a8a":"#64748b",
                boxShadow:authMode===m?"0 1px 5px rgba(0,0,0,0.10)":"none",transition:"all 0.15s"}}>{l}</button>
          ))}
        </div>
        {authMode==="register"&&(
          <div style={{marginBottom:14}}>
            ...
            <input value={authForm.name} onChange={e=>setAuthForm(f=>({...f,name:e.target.value}))} placeholder="Your full name"
              style={{width:"100%",padding:"10px 12px",border:"1.5px solid #e2e8f0",borderRadius:7,fontSize:14,boxSizing:"border-box",outline:"none"}}/>
          </div>
        )}
        <div style={{marginBottom:14}}>
          ...
          <input type="email" value={authForm.email} onChange={e=>setAuthForm(f=>({...f,email:e.target.value}))} placeholder="you@email.com"
            style={{width:"100%",padding:"10px 12px",border:"1.5px solid #e2e8f0",borderRadius:7,fontSize:14,boxSizing:"border-box",outline:"none"}}/>
        </div>
        <div style={{marginBottom:authErr?"12px":"20px"}}>
          ...
          <input type="password" value={authForm.password} onChange={e=>setAuthForm(f=>({...f,password:e.target.value}))}
            onKeyDown={e=>e.key==="Enter"&&handleAuth()} placeholder="Minimum 6 characters"
            style={{width:"100%",padding:"10px 12px",border:"1.5px solid #e2e8f0",borderRadius:7,fontSize:14,boxSizing:"border-box",outline:"none"}}/>
        </div>
        {authErr&&<div style={{background:"#fef2f2",border:"1px solid #fecaca",color:"#dc2626",padding:"9px 12px",borderRadius:6,fontSize:13,marginBottom:16,lineHeight:1.4}}>{authErr}</div>}
        <button onClick={handleAuth}
          style={{width:"100%",padding:"12px",background:"linear-gradient(135deg,#1e3a8a,#1d4ed8)",color:"white",border:"none",borderRadius:8,fontSize:15,fontWeight:800,cursor:"pointer"}}>
        ...
        <div style={{textAlign:"center",marginTop:14,fontSize:12,color:"#94a3b8"}}>
          ...
          <button onClick={()=>setAuthMode(authMode==="login"?"register":"login")}
            style={{background:"none",border:"none",color:"#1d4ed8",cursor:"pointer",fontWeight:700,fontSize:12,textDecoration:"underline"}}>
            {authMode==="login"?"Register for free":"Sign in instead"}
          </button>
        </div>
      </div>
    </div>
        ...
        <div style={{background:"linear-gradient(135deg,#0f172a,#1e3a8a)",padding:"11px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{color:"white"}}>
              <div style={{fontWeight:700,fontSize:15}}>CUET (UG) 2026 — English (101)</div>
              <div style={{fontSize:11,opacity:0.55}}>Mock Test Practice Platform</div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <button onClick={()=>setShowKeySettings(s=>!s)}
              style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",color:"white",padding:"6px 14px",borderRadius:5,cursor:"pointer",fontSize:12,fontWeight:600}}>⚙ API Key</button>
            <button onClick={handleLogout}
              style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",color:"white",padding:"6px 14px",borderRadius:5,cursor:"pointer",fontSize:12,fontWeight:600}}>Logout</button>
          </div>
        </div>
        {showKeySettings&&(
          <div style={{background:"#fff7ed",borderBottom:"2px solid #fed7aa",padding:"14px 20px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
            <span style={{fontSize:13,fontWeight:700,color:"#9a3412"}}>Update Anthropic API Key:</span>
            <input value={setupKey} onChange={e=>setSetupKey(e.target.value)} placeholder="sk-ant-api03-..."
              style={{flex:1,minWidth:280,padding:"8px 12px",border:"1.5px solid #fed7aa",borderRadius:6,fontSize:13,fontFamily:"monospace",outline:"none"}}/>
            {setupErr&&<span style={{fontSize:12,color:"#dc2626"}}>{setupErr}</span>}
            <button onClick={saveApiKey}
              style={{padding:"8px 16px",background:"#ea580c",color:"white",border:"none",borderRadius:5,cursor:"pointer",fontSize:13,fontWeight:700}}>Save Key</button>
            <button onClick={()=>{setShowKeySettings(false);setSetupErr("");}}
              style={{padding:"8px 14px",background:"transparent",border:"1px solid #d1d5db",borderRadius:5,cursor:"pointer",fontSize:13}}>Cancel</button>
          </div>
        )}
        <div style={{maxWidth:900,margin:"0 auto",padding:"22px 20px"}}>
          <div style={{marginBottom:18}}>
            <h1 style={{margin:0,fontSize:21,fontWeight:900,color:"#0f172a"}}>Welcome, {user?.name?.split(" ")[0]}</h1>
            <p style={{margin:"4px 0 0",color:"#64748b",fontSize:14}}>CUET English — Section I · Language Test (101)</p>
          </div>
          {history.length>0&&(
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:18}}>
              {[{v:history.length,l:"Tests Taken",c:"#1d4ed8"},{v:(best||0)+"%",l:"Best Score",c:"#16a34a"},{v:(avg||0)+"%",l:"Avg Score",c:"#d97706"},{v:totalDone,l:"Questions Done",c:"#7c3aed"}]
                .map(({v,l,c})=>(
                  <div key={l} style={{background:"white",borderRadius:9,padding:"14px 16px",boxShadow:"0 1px 4px rgba(0,0,0,0.07)",borderTop:`3px solid ${c}`}}>
                    <div style={{fontSize:26,fontWeight:900,color:c,lineHeight:1.1}}>{v}</div>
          ...
          <div style={{background:"white",borderRadius:11,padding:24,boxShadow:"0 1px 4px rgba(0,0,0,0.07)",marginBottom:18}}>
            ...
            <div style={{marginBottom:18}}>
              <div style={{fontSize:12,fontWeight:700,color:"#64748b",marginBottom:10,textTransform:"uppercase",letterSpacing:0.8}}>Test Mode</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
                {Object.entries(MODES).map(([k,m])=>(
                  <div key={k} onClick={()=>setMode(k)}
                    style={{border:`2px solid ${mode===k?"#1d4ed8":"#e2e8f0"}`,borderRadius:9,padding:"14px 12px",cursor:"pointer",
                      background:mode===k?"#eff6ff":"white",transition:"all 0.15s",userSelect:"none"}}>
            ...
            <div style={{marginBottom:22}}>
              <div style={{fontSize:12,fontWeight:700,color:"#64748b",marginBottom:10,textTransform:"uppercase",letterSpacing:0.8}}>Difficulty</div>
              <div style={{display:"flex",gap:10}}>
                {Object.entries(DIFFS).map(([k,d])=>(
                  <button key={k} onClick={()=>setDifficulty(k)}
                    style={{flex:1,padding:"11px 8px",border:`2px solid ${difficulty===k?d.color:"#e2e8f0"}`,borderRadius:8,cursor:"pointer",
                      background:difficulty===k?d.bg:"white",color:difficulty===k?d.color:"#374151",fontWeight:800,fontSize:14,transition:"all 0.15s"}}>
                    {d.label}
                    <div style={{fontSize:10,fontWeight:500,marginTop:2,opacity:0.75}}>{d.desc}</div>
                  </button>
                ))}
              </div>
            </div>
            {genError&&<div style={{background:"#fef2f2",border:"1px solid #fecaca",color:"#dc2626",padding:"10px 14px",borderRadius:6,fontSize:13,marginBottom:14,lineHeight:1.5}}>{genError}</div>}
            <button onClick={generateTest}
              style={{width:"100%",padding:"14px",background:"linear-gradient(135deg,#1e3a8a,#1d4ed8)",color:"white",border:"none",borderRadius:9,fontSize:16,fontWeight:900,cursor:"pointer"}}>
              Generate &amp; Start Test →
            </button>
          </div>
          {history.length>0?(
            <div style={{background:"white",borderRadius:11,padding:20,boxShadow:"0 1px 4px rgba(0,0,0,0.07)"}}>
              <h2 style={{margin:"0 0 14px",fontSize:16,fontWeight:800,color:"#0f172a"}}>Test History</h2>
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                  <thead><tr style={{background:"#f8fafc"}}>
                    {["Date & Time","Mode","Level","Score","Correct","Wrong","Skipped"].map(h=>(
                      <th key={h} style={{padding:"8px 10px",textAlign:"left",fontWeight:700,color:"#374151",borderBottom:"2px solid #e2e8f0",whiteSpace:"nowrap",fontSize:12}}>{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {history.map(t=>(
                      <tr key={t.id} style={{borderBottom:"1px solid #f1f5f9"}}>
                        <td style={{padding:"9px 10px",color:"#64748b",whiteSpace:"nowrap",fontSize:12}}>
                          {new Date(t.date).toLocaleDateString("en-IN",{day:"2-digit",month:"short"})}&nbsp;
                          {new Date(t.date).toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}
                        </td>
                        <td style={{padding:"9px 10px",textTransform:"capitalize",fontWeight:600}}>{t.mode}</td>
                        <td style={{padding:"9px 10px"}}><span style={{background:DIFFS[t.difficulty]?.bg,color:DIFFS[t.difficulty]?.color,padding:"2px 9px",borderRadius:12,fontSize:11,fontWeight:700}}>{t.difficulty}</span></td>
                        <td style={{padding:"9px 10px",fontWeight:900,color:t.percentage>=70?"#16a34a":t.percentage>=50?"#d97706":"#dc2626"}}>{t.score}/{t.maxScore} ({t.percentage}%)</td>
                        <td style={{padding:"9px 10px",color:"#16a34a",fontWeight:700}}>✓ {t.correct}</td>
                        <td style={{padding:"9px 10px",color:"#dc2626",fontWeight:700}}>✗ {t.wrong}</td>
                        <td style={{padding:"9px 10px",color:"#94a3b8"}}>{t.unattempted}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div style={{textAlign:"center",padding:"38px 20px",background:"white",borderRadius:11,boxShadow:"0 1px 4px rgba(0,0,0,0.07)"}}>
              <div style={{fontSize:42,marginBottom:12}}>📝</div>
              <div style={{fontWeight:800,fontSize:16,color:"#1e293b"}}>No tests taken yet</div>
              <div style={{fontSize:14,color:"#64748b",marginTop:5}}>Start your first test above to begin tracking performance</div>
            </div>
          )}
        </div>
      </div>
  ...
  // ═══════════════════════════════════════════════════════════
  //  EXAM SCREEN — AUTHENTIC NTA CBT INTERFACE
  // ═══════════════════════════════════════════════════════════
  if(screen==="exam"&&q){
      ...
      <div style={{minHeight:"100vh",background:"#d1d5db",fontFamily:"'Segoe UI',system-ui,sans-serif",fontSize:14,display:"flex",flexDirection:"column"}}>
        <div style={{background:"linear-gradient(135deg,#0f172a,#1e3a8a)",padding:"7px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{color:"white",fontSize:14,fontWeight:700}}>CUET (UG) — 2026 | Common University Entrance Test (Undergraduate)</div>
          </div>
        ...
        <div style={{background:"#bfdbfe",padding:"7px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"2px solid #93c5fd",flexShrink:0,flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <span style={{fontFamily:"'Courier New',monospace",fontSize:20,fontWeight:900,color:tColor,
              background:"white",padding:"3px 12px",borderRadius:3,border:`2px solid ${tColor}`,
              animation:timerRed?"blink 1s infinite":"none"}}>
              {fmtTime(timeLeft)}
            </span>
          </div>
        </div>
        <div style={{display:"flex",flex:1,overflow:"hidden",minHeight:0}}>
          <div style={{flex:1,padding:14,overflowY:"auto",background:"#f0f0f0"}}>
            <div style={{background:"white",border:"1px solid #d1d5db",borderRadius:3,padding:"9px 14px",marginBottom:10,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <span style={{background:TOPICS[q.topic]?.color+"18",color:TOPICS[q.topic]?.color||"#374151",fontSize:11,fontWeight:800,padding:"2px 8px",borderRadius:10,border:`1px solid ${TOPICS[q.topic]?.color||"#374151"}30`}}>
                  {TOPICS[q.topic]?.label||q.topic}
                </span>
              </div>
              <div style={{fontSize:12,color:"#374151"}}>
                <span style={{color:"#16a34a",fontWeight:700}}>+5</span> Correct &nbsp;
                <span style={{color:"#dc2626",fontWeight:700}}>-1</span> Wrong &nbsp;
                <span style={{color:"#94a3b8"}}>0</span> Unattempted
              </div>
            </div>
            {passage&&(
              <div style={{background:"#fefce8",border:"1px solid #fde68a",borderRadius:3,padding:"12px 15px",marginBottom:10}}>
                <div style={{fontSize:10,fontWeight:800,color:"#92400e",letterSpacing:1.5,marginBottom:7,textTransform:"uppercase"}}>
                  Read the Following Passage Carefully — {(passage.type||"").toUpperCase()}
                </div>
                <div style={{maxHeight:180,overflowY:"auto"}}>
                  <p style={{margin:0,lineHeight:1.9,color:"#1c1917",fontSize:13.5}}>{passage.text}</p>
                </div>
              </div>
            )}
            ...
            <div style={{background:"white",border:"1px solid #d1d5db",borderRadius:3,overflow:"hidden"}}>
              {["A","B","C","D"].map((opt,oi)=>{
                ...
                return(
                  <div key={opt} onClick={()=>setSelectedOpt(opt)}
                    style={{display:"flex",alignItems:"flex-start",gap:12,padding:"13px 16px",cursor:"pointer",
                      background:sel?"#eff6ff":"white",borderBottom:oi<3?"1px solid #f1f5f9":"none",
                      borderLeft:sel?"3px solid #1d4ed8":"3px solid transparent",transition:"all 0.1s",userSelect:"none"}}>
                    <div style={{width:20,height:20,borderRadius:"50%",flexShrink:0,marginTop:2,
                      border:sel?"2px solid #1d4ed8":"2px solid #9ca3af",background:sel?"#1d4ed8":"white",
                    ...
                    <span style={{fontSize:14,lineHeight:1.75,color:sel?"#1e40af":"#1e293b"}}>
                      <strong style={{marginRight:6,color:sel?"#1e40af":"#374151"}}>{opt}.</strong>{q.options?.[opt]||""}
                    </span>
                  </div>
                );
              })}
          ...
          <div style={{width:198,background:"white",borderLeft:"2px solid #d1d5db",display:"flex",flexDirection:"column",flexShrink:0,overflow:"hidden"}}>
            ...
            <div style={{padding:"9px 12px",borderBottom:"1px solid #f1f5f9",display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
              <div style={{width:30,height:30,borderRadius:"50%",background:"linear-gradient(135deg,#1e3a8a,#1d4ed8)",display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontWeight:900,fontSize:14,flexShrink:0}}>
            ...
            <div style={{flex:1,overflowY:"auto",padding:10}}>
              <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:5,marginBottom:14}}>
                {questions.map((qi,idx)=>(
                  <div key={qi.id} onClick={()=>goTo(idx)}
                    style={{...qstyle(qi.id),outline:idx===currentQ?"3px solid #f59e0b":"none",outlineOffset:1}}>
                    {idx+1}
                  </div>
                ))}
              </div>
              <div style={{fontSize:11,color:"#374151"}}>
                <div style={{fontWeight:700,marginBottom:6,fontSize:10,textTransform:"uppercase",letterSpacing:0.5,color:"#94a3b8"}}>Legend</div>
                {[["not_visited","Not Visited"],["not_answered","Not Answered"],["answered","Answered"],["review","Marked for Review"],["answered_review","Answered & Marked"]].map(([s,l])=>(
                  <div key={s} style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}>
                    <div style={{width:16,height:16,background:QS[s].bg,border:`2px solid ${QS[s].border}`,borderRadius:2,flexShrink:0}}/>
            ...
            <div style={{padding:10,borderTop:"1px solid #e2e8f0",flexShrink:0}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}>
                <div style={{background:"#dcfce7",padding:"7px",borderRadius:5,textAlign:"center"}}>
                  <div style={{fontWeight:900,fontSize:18,color:"#16a34a"}}>{answeredCount}</div>
                  <div style={{fontSize:9,color:"#166534",textTransform:"uppercase",letterSpacing:0.5}}>Answered</div>
                </div>
                <div style={{background:"#fef3c7",padding:"7px",borderRadius:5,textAlign:"center"}}>
                  <div style={{fontWeight:900,fontSize:18,color:"#d97706"}}>{questions.length-answeredCount}</div>
                  <div style={{fontSize:9,color:"#92400e",textTransform:"uppercase",letterSpacing:0.5}}>Remaining</div>
                </div>
              </div>
              <button onClick={()=>{if(window.confirm(`Submit test?\n\n${answeredCount} answered · ${questions.length-answeredCount} unattempted\n\nThis cannot be undone.`))doSubmit();}}
                style={{width:"100%",padding:"9px",background:"linear-gradient(135deg,#7f1d1d,#dc2626)",color:"white",border:"none",borderRadius:4,fontWeight:900,fontSize:13,cursor:"pointer",letterSpacing:0.5}}>
                SUBMIT TEST
              </button>
            </div>
          </div>
        ...
        <div style={{background:"white",borderTop:"2px solid #d1d5db",padding:"9px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,flexWrap:"wrap",gap:8}}>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <button onClick={()=>currentQ>0&&goTo(currentQ-1)} disabled={currentQ===0}
              style={{padding:"8px 16px",background:currentQ===0?"#f8fafc":"#e2e8f0",border:"1px solid #d1d5db",borderRadius:3,
                cursor:currentQ===0?"not-allowed":"pointer",fontSize:13,color:currentQ===0?"#94a3b8":"#374151",fontWeight:600}}>◄ Back</button>
            <button onClick={clearResponse}
              style={{padding:"8px 14px",background:"transparent",border:"none",color:"#dc2626",cursor:"pointer",fontSize:13,textDecoration:"underline",fontWeight:600}}>Clear Response</button>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={markAndNext}
              style={{padding:"9px 16px",background:"linear-gradient(135deg,#4c1d95,#7c3aed)",color:"white",border:"none",borderRadius:3,cursor:"pointer",fontSize:13,fontWeight:700}}>
              Mark for Review &amp; Next ►
            </button>
            <button onClick={saveAndNext}
              style={{padding:"9px 22px",background:"linear-gradient(135deg,#1e3a8a,#1d4ed8)",color:"white",border:"none",borderRadius:3,cursor:"pointer",fontSize:14,fontWeight:900}}>
              Save &amp; Next ►
            </button>
          </div>
        </div>
        ...
        <div style={{maxWidth:820,margin:"0 auto",padding:"22px 20px"}}>
          <div style={{background:"white",borderRadius:12,padding:"28px 24px",boxShadow:"0 4px 20px rgba(0,0,0,0.08)",marginBottom:18,textAlign:"center",borderTop:`5px solid ${sc}`}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,maxWidth:480,margin:"0 auto"}}>
              {[{v:results.correct,l:"Correct",c:"#16a34a",p:`+${results.correct*5}`},{v:results.wrong,l:"Wrong",c:"#dc2626",p:`-${results.wrong}`},{v:results.unattempted,l:"Skipped",c:"#94a3b8",p:"0"}]
                .map(({v,l,c,p})=>(
                  <div key={l} style={{background:"#f8fafc",borderRadius:9,padding:"12px 8px"}}>
                    <div style={{fontSize:28,fontWeight:900,color:c}}>{v}</div>
                    <div style={{fontSize:12,color:"#64748b"}}>{l}</div>
                    <div style={{fontSize:11,color:c,fontWeight:700,marginTop:2}}>{p} marks</div>
                  </div>
                ))}
            </div>
          </div>
          <div style={{background:"white",borderRadius:11,padding:22,boxShadow:"0 1px 4px rgba(0,0,0,0.07)",marginBottom:14}}>
            ...
            {Object.entries(results.topicStats).map(([t,s])=>{
              ...
              return(
                <div key={t} style={{marginBottom:16}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <span style={{background:tc?.color+"18",color:tc?.color||"#374151",fontSize:10,fontWeight:800,padding:"2px 7px",borderRadius:10}}>{tc?.abbr||t}</span>
                    ...
                    <div style={{display:"flex",gap:10,alignItems:"center"}}>
                      ...
                      <span style={{fontSize:14,fontWeight:900,color:bc,minWidth:40,textAlign:"right"}}>{acc}%</span>
                    </div>
                  </div>
                  <div style={{background:"#f1f5f9",borderRadius:100,height:9,overflow:"hidden"}}>
                    <div style={{height:"100%",borderRadius:100,background:bc,width:acc+"%",transition:"width 0.7s ease"}}/>
                  </div>
          ...
          <div style={{background:"white",borderRadius:11,padding:20,boxShadow:"0 1px 4px rgba(0,0,0,0.07)",marginBottom:16}}>
            ...
            {results.questionResults.map((qr,i)=>{
              ...
              const sbg={correct:"#dcfce7",wrong:"#fee2e2",unattempted:"#f8fafc"}[qr.status];
              const psg=qr.passage_id?passages.find(p=>p.id===qr.passage_id):null;
              return(
                <div key={qr.id} style={{border:`1px solid ${sc2}30`,borderRadius:7,marginBottom:8,overflow:"hidden"}}>
                  <div onClick={()=>setReviewOpen(isOpen?null:qr.id)}
                    style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",cursor:"pointer",background:sbg,userSelect:"none"}}>
                    ...
                    <div style={{flex:1,fontSize:13,color:"#1e293b",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{(qr.question||"").slice(0,85)}{(qr.question||"").length>85?"...":""}</div>
                    <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                      ...
                      <span style={{fontSize:11,fontWeight:700,color:sc2,textTransform:"uppercase",minWidth:60}}>{qr.status}</span>
                      <span style={{fontSize:12,color:sc2}}>{isOpen?"▲":"▼"}</span>
                    </div>
                  </div>
                  {isOpen&&(
                    <div style={{padding:"14px 16px",borderTop:`1px solid ${sc2}20`,background:"white"}}>
                      {psg&&<div style={{background:"#fefce8",border:"1px solid #fde68a",borderRadius:5,padding:"10px 12px",marginBottom:12,fontSize:12,lineHeight:1.8,color:"#1c1917",maxHeight:120,overflowY:"auto"}}>{psg.text}</div>}
                      <p style={{margin:"0 0 12px",fontSize:14,fontWeight:600,color:"#1e293b",lineHeight:1.7}}>{qr.question}</p>
                      {["A","B","C","D"].map(opt=>{
                        const correct=opt===qr.correct, userWrong=opt===qr.userAnswer&&!correct;
                        return(
                          <div key={opt} style={{padding:"8px 12px",borderRadius:5,marginBottom:5,fontSize:13,
                            background:correct?"#dcfce7":userWrong?"#fee2e2":"#f8fafc",
                            border:`1px solid ${correct?"#16a34a":userWrong?"#dc2626":"#e2e8f0"}`,
                            color:correct?"#14532d":userWrong?"#7f1d1d":"#374151"}}>
                            <strong>{opt}.</strong> {qr.options?.[opt]}{correct?" ✓":""}{userWrong?" ✗ Your Answer":""}
                          </div>
                        );
                      })}
                      {qr.explanation&&<div style={{background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:5,padding:"10px 12px",marginTop:10,fontSize:12.5,color:"#1e40af",lineHeight:1.6}}>
          ...
          <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
            <button onClick={getAdvisory} disabled={loadingAdv}
              style={{flex:2,minWidth:200,padding:"14px",background:loadingAdv?"#94a3b8":"linear-gradient(135deg,#4c1d95,#7c3aed)",
                color:"white",border:"none",borderRadius:9,fontSize:15,fontWeight:900,cursor:loadingAdv?"not-allowed":"pointer"}}>
              {loadingAdv?"Generating Coaching Advisory...":"Get AI Coaching Advisory →"}
            </button>
            <button onClick={()=>setScreen("dashboard")}
              style={{flex:1,minWidth:140,padding:"14px",background:"white",color:"#1d4ed8",border:"2px solid #1d4ed8",borderRadius:9,fontSize:14,fontWeight:700,cursor:"pointer"}}>
              Take New Test
            </button>
          </div>
        </div>
      ...
      <div style={{maxWidth:760,margin:"0 auto",padding:"26px 20px"}}>
        {results&&(
          <div style={{display:"flex",gap:10,marginBottom:18,flexWrap:"wrap"}}>
            {[{v:`${results.score}/${results.maxScore}`,l:"Score",c:"#1d4ed8"},{v:`${results.percentage}%`,l:"Percentage",c:results.percentage>=70?"#16a34a":results.percentage>=50?"#d97706":"#dc2626"},{v:results.correct,l:"Correct",c:"#16a34a"},{v:results.wrong,l:"Wrong",c:"#dc2626"}]
              .map(({v,l,c})=>(
                <div key={l} style={{background:"white",borderRadius:8,padding:"10px 16px",boxShadow:"0 1px 4px rgba(0,0,0,0.07)",textAlign:"center",minWidth:80}}>
        ...
        <div style={{background:"white",borderRadius:13,padding:"28px 30px",boxShadow:"0 4px 20px rgba(0,0,0,0.08)",borderLeft:"5px solid #7c3aed",marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,paddingBottom:16,borderBottom:"1px solid #f1f5f9"}}>
            <div style={{width:42,height:42,borderRadius:"50%",background:"linear-gradient(135deg,#4c1d95,#7c3aed)",display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontSize:20,flexShrink:0}}>🎓</div>
            <div>
              <div style={{fontWeight:900,fontSize:16,color:"#1e293b"}}>Coaching Advisory — {user?.name}</div>
              <div style={{fontSize:12,color:"#94a3b8",marginTop:2}}>Based on your latest CUET English (101) test performance</div>
            </div>
          </div>
        ...
        <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
          <button onClick={generateTest}
            style={{flex:1,minWidth:180,padding:"13px",background:"linear-gradient(135deg,#1e3a8a,#1d4ed8)",color:"white",border:"none",borderRadius:9,fontSize:15,fontWeight:900,cursor:"pointer"}}>
            Start Next Test →
          </button>
          <button onClick={()=>setScreen("dashboard")}
            style={{flex:1,minWidth:140,padding:"13px",background:"white",color:"#374151",border:"2px solid #e2e8f0",borderRadius:9,fontSize:14,fontWeight:700,cursor:"pointer"}}>
            ← Dashboard
          </button>
        </div>
      </div>
